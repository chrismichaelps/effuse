...
**Effuse** へようこそ。TypeScript と [Effect](https://effect.website) を使用して構築されたモダンなリアクティブ UI フレームワークです。

## Effuse とは？

Effuse は、モダンなフロントエンド開発のベストプラクティスを組み合わせた、シグナルベースの UI フレームワークです：

- **細粒度のリアクティビティ** — 変更されたデータに依存する DOM ノードのみが更新されます
- **型安全なコンポーネント** — 型推論を備えた完全な TypeScript サポート
- **Effect による強化** — 堅牢な非同期処理とエラーハンドリングのため Effect 上に構築
- **コンポーザブルなアーキテクチャ** — 拡張性のためのレイヤーとプラグインシステム
