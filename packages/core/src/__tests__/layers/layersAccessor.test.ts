...
describe('LayersAccessor — comprehensive regression tests', () => {
	...
	describe('resolveLayersAccessor — runtime behavior', () => {
		...
		it('should expose extended layers by name in list form', () => {
			const authSvc = { token: 'abc' };
			const featureSvc = { flag: 'billing-beta' };
			...
			const layerRegistry = createMockLayerRegistry(
				{ authBase: resolvedAuth, featureShell: resolvedFeature },
				{ authSvc, featureSvc }
			);
			const store = {
				propsRegistry,
			...
			const accessor = resolveLayersAccessor([featureLayer] as const);

			expect(Object.keys(accessor)).toEqual(['authBase', 'featureShell']);
			expectTypeOf<typeof accessor.authBase.services.authSvc>().toEqualTypeOf<{
				token: string;
			}>();
			expect(
				runWithLayerContext(store, () => accessor.authBase.props.mode)
			).toBe(authMode);
			expect(
				runWithLayerContext(store, () => accessor.authBase.services.authSvc)
			).toBe(authSvc);
			expect(
				runWithLayerContext(
					store,
					() => accessor.featureShell.services.featureSvc
				)
			).toBe(featureSvc);
		});

		it('should keep alias records explicit when the aliased layer extends another layer', () => {
			const authLayer = defineLayer({
				name: 'platformAuth',
			...
			const featureLayer = defineLayer({
				...
				extends: [authLayer],
				services: { featureSvc: () => ({ enabled: true }) },
			});
			...
			expect(Object.keys(accessor)).toEqual(['feature']);
			expect('platformAuth' in accessor).toBe(false);
			expect('platformFeature' in accessor).toBe(false);
			expectTypeOf<keyof typeof accessor>().toEqualTypeOf<'feature'>();
		});
		...
		it('should key alias records by local alias instead of layer name', () => {
			...
			expect(runWithLayerContext(store, () => accessor.auth.props.mode)).toBe(
				modeSignal
			);
		});
		...
		it('should expose props getter that lazily resolves from global context', () => {
			...
			expect(runWithLayerContext(store, () => accessor.theme.props.mode)).toBe(
				modeSignal
			);
		});
		...
		it('should return cached service — factory NOT called on repeated access', () => {
			...
			const first = runWithLayerContext(
				store,
				() => accessor.myLayer.services.myService
			);
			const second = runWithLayerContext(
				store,
				() => accessor.myLayer.services.myService
			);
		...
		it('should handle multiple layers with distinct service keys', () => {
			...
			const store = {
				propsRegistry,
				layerRegistry,
				layers: [resolvedAuth, resolvedLog, resolvedStore],
			};
			...
			expect(
				runWithLayerContext(store, () => accessor.log.services.logSvc)
			).toBe(logSvc);
			expect(
				runWithLayerContext(store, () => accessor.store.services.storeSvc)
			).toBe(storeSvc);
		});
	...
	describe('LayersAccessor — type inference per layer', () => {
		it('should infer distinct service types per layer key', () => {
			...
			const store = {
				propsRegistry,
				layerRegistry,
				layers: [resolvedStr, resolvedNum],
			};

			const accessor = resolveLayersAccessor([strLayer, numLayer]);

			const strResult = runWithLayerContext(
				store,
				() => accessor.strLayer.services.strService
			);
			const numResult = runWithLayerContext(
				store,
				() => accessor.numLayer.services.numService
			);
		...
		it('should not conflate service types across layers — auth vs log', () => {
			...
			const a = runWithLayerContext(
				store,
				() => accessor.auth.services.authSvc
			);
		...
		it('should infer service types from alias record keys', () => {
			...
			expectTypeOf<typeof accessor.auth.services.authSvc>().toEqualTypeOf<{
				token: string;
			}>();
			expectTypeOf<typeof accessor.logger.services.logSvc>().toEqualTypeOf<{
				log: () => undefined;
			}>();
		});
		...
		it('should infer layer props from list and alias accessors', () => {
			...
			expectTypeOf<typeof listAccessor.typedTheme.props.mode>().toEqualTypeOf<
				typeof modeSignal
			>();
			expectTypeOf<
				typeof listAccessor.typedTheme.props.density
			>().toEqualTypeOf<typeof densitySignal>();
			expectTypeOf<typeof aliasAccessor.theme.props.mode>().toEqualTypeOf<
				typeof modeSignal
			>();
			expectTypeOf<typeof aliasAccessor.theme.services.theme>().toEqualTypeOf<{
				current: () => string;
			}>();
		});
	});
	...
	describe('createHookContext — layers accessor integration', () => {
		...
		it('should resolve services via lazy getter', () => {
			...
			expect(
				runWithLayerContext(store, () => ctx.layers.auth.services.authSvc)
			).toBe(authSvc);
		});
	});
	...
	describe('edge cases', () => {
		it('should handle layer with single character name', () => {
			...
			const resolved = createResolvedLayer({
				name: 'a',
				provides: { xSvc: () => svc },
			});
			...
			expect(runWithLayerContext(store, () => accessor.a.services.xSvc)).toBe(
				svc
			);
		});
		...
		it('should handle layer with numeric string name', () => {
			...
			const accessor = resolveLayersAccessor([layer]);
			expect(
				runWithLayerContext(store, () => accessor['123'].services.numSvc)
			).toBe(svc);
		});
		...
		it('should handle layer with camelCase name', () => {
			...
			const accessor = resolveLayersAccessor([layer]);
			expect(
				runWithLayerContext(store, () => accessor.myDbLayer.services.dbService)
			).toBe(svc);
		});
		...
		it('should handle multiple services in a single layer', () => {
			...
			expect(
				runWithLayerContext(store, () => accessor.combined.services.authSvc)
			).toBe(authSvc);
			expect(
				runWithLayerContext(store, () => accessor.combined.services.userSvc)
			).toBe(userSvc);
		});
	});
});
