		...
		if (
			...
			if (arg && ts.isObjectLiteralExpression(arg)) {
				...
				for (const prop of arg.properties) {
					...
					if (
						propName === 'deriveProps' &&
						ts.isArrowFunction(prop.initializer)
					) {
						propsType = extractDerivePropsType(prop.initializer, sourceFile);
					}
					if (
						propName === 'provides' &&
						ts.isObjectLiteralExpression(prop.initializer)
...
function extractDerivePropsType(
	...
	if (ts.isParenthesizedExpression(body)) {
		...
		if (ts.isObjectLiteralExpression(inner)) {
			return extractPropsFromObjectLiteral(inner, sourceFile);
		}
	}

	if (ts.isObjectLiteralExpression(body)) {
		return extractPropsFromObjectLiteral(body, sourceFile);
	}
	...
	for (const prop of node.properties) {
		...
		if (ts.isCallExpression(init)) {
			...
			if (funcName === 'signal') {
				...
				props.push(`${propName}: ReadonlySignal<unknown>`);
			}
		} else if (ts.isPropertyAccessExpression(init)) {
			props.push(`${propName}: Signal<unknown>`);
		}
	}
